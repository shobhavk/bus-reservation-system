...
@routes.get('/', response_model=List[schema.Payment])
# def all(db: Session = Depends(get_db), current_user: schemas.User = Depends(oauth2.get_current_user)):
def all(db: Session = Depends(get_db)):
    return database_manager.index(db)

@routes.get('/{id}', status_code=200, response_model=schema.Payment)
...
def update(id: int, request: schema.Payment, db: Session = Depends(get_db)):
